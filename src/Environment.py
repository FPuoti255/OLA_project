    ...
    def compute_nodes_activation_probabilities(self, product_prices):
        ...
        for node in tqdm(range(NUM_OF_PRODUCTS),position = 0, desc='node' ,leave=False):
            for _ in tqdm(range(k), position=1, desc='k'):                
                active_nodes = self.generate_live_edge_graph(seed = node, 
                                                        product_prices= product_prices, show_plots = False)
                z[node][active_nodes] += 1

        return z / k


    def generate_live_edge_graph(self, seed : int,
                                    product_prices,
                                    show_plots = False):
        ...
        while list(white_nodes):
            ...
            if np.random.binomial(n=1, p = np.tanh(self.users_reservation_prices[primary_product] / product_prices[primary_product])):
