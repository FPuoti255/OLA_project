...
class Non_Stationary_Environment:
    def __init__(self, users_reservation_prices, product_functions_idxs, click_probabilities, users_alpha, horizon):
    ...
    def round_step6(self, pulled_arm, num_sold_items):
        current_phase = int(self.t / self.phase_size)

            
        arm = renormalize(pulled_arm)

        prd_function_idx = self.product_functions_idxs[current_phase]
        print(prd_function_idx)
        concentration_parameters = np.array([ self.functions_dict[prd_function_idx[i]](arm[i]) for i in range(len(pulled_arm))])
        ...
        current_users_alpha = np.sum(self.users_alpha[current_phase], axis=0)[1:]
        ...
        sold_items = np.multiply(np.divide(reward, current_users_alpha), num_sold_items[current_phase])
